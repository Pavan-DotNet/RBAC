...
{
    public class SCHSController : Controller
    {
        // GET: SCHS
        ...
        public JsonResult Search(string EmiratesId, string UserAgent)
        {
            ...
            try
            {
                ...
                SCHSDetails.Root root = JsonConvert.DeserializeObject<SCHSDetails.Root>(response.Content);
                if (root.success)
                {
                    flag = 1;
        ...
        private static RestResponse CallInfoAPICall(string emirateID)
        {
            ...
            string apiURL = ConfigurationManager.AppSettings["SCHS_URL"].ToString();
