...
{
    public class PFMembershipController : Controller
    {
        public static string Title = "طلبات عضويات الأسر المنتجة";
        // GET: PFMembership
        ...
        public ActionResult Search(string postdata, string UserAgent)
        {
            ...
            try
            {
                ...
                var uri = new Uri($"{ConfigurationManager.AppSettings["CRMApiUrl"]}/Services/GetPFApplicationsByEID?EID={postdata}");
                byte[] _response = client.DownloadData(uri);
                if (_response != null)
                {
                    var responseObj = JsonConvert.DeserializeObject<MOCDResponse<List<PFMembership>>>(Encoding.UTF8.GetString(_response));
                    if (responseObj.Code == MOCDErroCode.Success)
                    {
                        if (responseObj.Content.Count > 0)
                        {
                            ...
                            LogIntegrationDetails.LogSerilog(input, JsonConvert.SerializeObject(responseObj.Content), EntityCodes.PFM.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                        }
                        else
                        {
                            flag = 2;
                            ...
                            LogIntegrationDetails.LogSerilog(input, "لا يوجد سجلات", EntityCodes.PFM.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                        }
                    }
                    else
                    {
                        ...
                        LogIntegrationDetails.LogSerilog(input, responseObj.ResponseDescriptionAr, EntityCodes.PFM.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                LogIntegrationDetails.LogSerilog(input, ex.Message, EntityCodes.PFM.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
            }
