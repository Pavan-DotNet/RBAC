...
{
    public class EdaadController : Controller
    {
        public static string Title = "حضور برنامج إعداد من خلال المنصة الإلكترونية";
        ...
        public ActionResult Search(string postdata, string UserAgent)
        {
            ...
            try
            {
                ...
                var uri = new Uri($"{ConfigurationManager.AppSettings["CRMApiUrl"]}/Services/GetEdaadAttendanceByEID?EID={postdata}");
                byte[] _response = client.DownloadData(uri);
                if (_response != null)
                {
                    var responseObj = JsonConvert.DeserializeObject<MOCDResponse<List<EdaadInfo>>>(Encoding.UTF8.GetString(_response));
                    if (responseObj.Code == MOCDErroCode.Success)
                    {
                        if (responseObj.Content.Count > 0)
                        {
                            ...
                            LogIntegrationDetails.LogSerilog(input, JsonConvert.SerializeObject(responseObj.Content), EntityCodes.EDD.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                        }
                        else
                        {
                            flag = 2;
                            ...
                            LogIntegrationDetails.LogSerilog(input, "لا يوجد سجلات", EntityCodes.EDD.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                        }
                    }
                    else
                    {
                        ...
                        LogIntegrationDetails.LogSerilog(input, responseObj.ResponseDescriptionAr, EntityCodes.EDD.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                LogIntegrationDetails.LogSerilog(input, ex.Message, EntityCodes.EDD.ToString(), Title, DateTime.Now.ToString(), string.Empty, UserAgent, User.Identity.Name);
            }
